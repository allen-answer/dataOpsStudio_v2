# AGENT_CORE — DataOps Studio 2.0 宪法

> Agent 每次运行前自动加载。**速记 + 索引,不是规则原文**。
> 权威定义在 `AGENT_CONTRACT_v2.md`(开工前读)。两份漂移以 contract 为准。
> 本文件目的:30 秒内告诉你 — 哪不能碰、哪要查 contract、哪要先问人。
...
- 新规则 / 改规则 → **先改 contract** 对应章节,再改 CORE 速记
- CORE 永远是 contract 的**子集 + 索引**,不能出现 contract 里没有的新规则
- 字节硬限 **< 12KB**
