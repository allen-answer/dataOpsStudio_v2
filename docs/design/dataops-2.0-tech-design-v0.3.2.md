...
- lineage 解析结果**不持久化**——每次跑重算,只在 workload(scenario / workflow 节点 / batch 导出)时落 JSON。
...
| **lineage 解析结果** | **不迁**(1.x 本就不持久化,每次重算,见 §2.4) |
...
- lineage 解析结果不迁(无 aspect 收敛,无历史 aspect 数据可迁)
