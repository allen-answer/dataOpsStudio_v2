...
| 任务 | 负责 | 依赖 | 状态 | PR |
|---|---|---|---|---|
| Datasource polish:`PUT/DELETE /datasources/{id}`(409 引用检查)+ operation_policy(8×allow_*)+ worker 结构化 error code | Codex | T5 合并(schemas.py 释放)+ DM adapter 合并(worker.py 释放) | 进行中(实现 + 单测完成;待 PR CI 全绿) | — |
