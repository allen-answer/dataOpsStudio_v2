...
| 任务 | 负责 | 依赖 | 状态 | PR |
|---|---|---|---|---|
| Wave 5B GA 前端收口 —— Tailwind darkMode 键失配修复(`[data-theme]`→`[data-mode]`,四变体逐个目检)+ UX nit 包(SQL 结果区 scrollIntoView / 数据源表单 inline 必填校验 / 确认按钮 in-flight disable / skipAuthRedirect 端点真会话过期主动登出)| Claude Opus 4.8 | — | PR 待审 |
