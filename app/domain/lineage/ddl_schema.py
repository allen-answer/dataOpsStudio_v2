...
★ 预处理层的定位(改这个模块之前先读这段):
  ...
  **只在高置信度时动手,拿不准就原样交给 sqlglot 判。** 这一层不是"我能正确切分
  任意 SQL"的通用词法器 —— 那条路的长尾是无穷的,而且每错一次都是静默出错。
  三条价值判断,按优先级排序:

  1. **绝不能比不用这个功能更差** —— 任何情况下 DDL 都不该让血缘结果劣于不提供
     DDL 时的基线(见 tests 里的 test_ddl_never_degrades_lineage_versus_no_ddl,
     那条是长期护栏)。
  2. **宁可少认,不可错认** —— 丢弃一个条目 / 一张表之前必须有把握。多认一个幻影
     列只是多一条没人引用的元数据;少认一个真列则直接让该列的血缘消失。
     所以拿不准就放它过去。
  3. **失败必须可见且指向正确** —— 静默丢弃是最坏结果。跳过原因分开计数
     (``SKIP_*``),摘要在合并处产出,让用户能定位**真实**原因。

  推论:预过滤**必须**拦下的只有"进了 sqlglot 会挂死 / 会产出幻影列"的那几种形状,
  其余标准约束 sqlglot 自己就能正确排除。别再往关键字集合里加词 —— 那正是
  ``period`` 被整列吞掉的成因。

关于"为什么不复用包内已有的切分器":``segments.py`` 的 ``_split_top_level_statements``
/ ``_split_top_level`` 只认单引号、不认注释 / 反引号 / 方括号 / 美元引号 / 反斜杠
转义,能力严格弱于本模块所需(本模块要在 DDL 原文上精确定位括号与逗号)。复用它
等于把这里已经修好的一批缺陷再引回来。反向统一(让 segments 用这套)会动到 L-3
存储过程解析的既有行为,不在本次范围 —— 留作独立议题。
"""
